...
from network_architectures import Baseline_VGGClassifier


class ClassifierNetworkGraph:
    def __init__(self, input_x, target_placeholder, dropout_rates,
                 batch_size=100, num_channels=1, n_classes=100, is_training=True, augment_rotate_flag=True,
                 tensorboard_use=False, use_batch_normalization=False, strided_dim_reduction=False,
                 network_name='Baseline_VGGClassifier', pool_size=(3,3), kernel_size=[5,5], num_fully_conn_layers=2):
        ...
        if network_name == "Baseline_VGGClassifier":
            self.c = Baseline_VGGClassifier(self.batch_size, name="classifier_neural_network",
                                   batch_norm_use=use_batch_normalization, num_channels=num_channels,
                                   num_classes=n_classes, layer_stage_sizes=[64, 128, 256], pool_size=(3,3), kernel_size=[5,5], num_fully_conn_layers=2)
        elif network_name == "FCCLayerClassifier":
    ...
    def train(self, losses, learning_rate=1e-3, beta1=0.9):
        ...
        with tf.control_dependencies(update_ops):
            ...
            self.c.variables = MaxNorm(self.c.variables)
