...
class MTLClassifier:
    def __init__(self, batch_size, layer_stage_sizes, name, num_classes, num_channels=1, batch_norm_use=False,
                 inner_layer_depth=2, strided_dim_reduction=True,branch=3):
    ...
    def __call__(self, image_input, training=False, dropout_rate=0.0):
        ...
        with tf.variable_scope(self.name, reuse=self.reuse):
                ...
                for i in range(len(self.layer_stage_sizes)):
                    with tf.variable_scope('conv_stage_{}'.format(i)):
                        if (i < branch):
                            ...
                            if (ok):
                                outputs1 = outputs
                                layer_features1 = layer_features1
                                ok = 0
                            for j in range(self.inner_layer_depth):
                                with tf.variable_scope('conv_{}_{}'.format(i, j)):
                                    if (j == self.inner_layer_depth-1) and self.strided_dim_reduction:
                                    ...
                                    outputs1 = tf.layers.conv2d(outputs1, self.layer_stage_sizes[i], [3, 3],
                                                               strides=(stride, stride),
                                                               padding='SAME', activation=None)
                                    outputs1 = leaky_relu(outputs1, name="leaky_relu{}".format(i))
            ...
            c_conv_encoder1 = tf.layers.dense(c_conv_encoder1, units=self.num_classes)
        ...
        if not self.build_completed:
            self.build_completed = True
            count_parameters(self.variables, "MTLNet")
        ...
        #Aux Task1
        results_From_Network.update({'auxTask1':(c_conv_encoder1, layer_features1)})
